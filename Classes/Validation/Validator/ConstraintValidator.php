...
class ConstraintValidator extends Validator\GenericObjectValidator implements Validator\ValidatorInterface
{
    ...
    protected function ___mergeErrors($errors)
    {
        ...
        foreach ($errors as $error) {
            $localizedFieldName = \TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate(
                $this->currentPropertyName,
                'SfRegister'
            );

            $errorMessage = $error->getMessage();
            $localizedErrorCode = \TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('error_'
                . $error->getCode(), 'SfRegister');
